...
export class DatabaseService {
  ...
  async openDatabase() {
    try {
      // Adiciona o argumento `readonly` (false no exemplo)
  ...
  async getClientes(): Promise<Cliente[]> {
    ...
    try {
      ...
      // Verifica se `result.values` é definido
      return result.values ? result.values.map((row: { id: number; nome: string; cpf: string; telefone: string; email: string; dataCadastro: string; expanded: number }) => ({
        id: row.id,
        nome: row.nome,
        cpf: row.cpf,
